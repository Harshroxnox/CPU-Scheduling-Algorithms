...
// it asks whether p1 < p2 correct conditions for max attr such as rr and reverse conditions for min attr such as bt, at, pid
struct ready_queue_compare {
    bool operator()(process& p1, process& p2){
        
        if(p1.response_ratio < p2.response_ratio){
            return true;
        }else if(p1.response_ratio > p2.response_ratio){
            return false;
...
void calc_attributes(vector<process>& list){
    ...
    while(true){
        ...
        // process
        if(!ready_queue.empty()){
            //calc response ratio of processes in ready queue
            vector<process> list_ready_queue;
            while(!ready_queue.empty()){
                int pid = ready_queue.top().pid;
                for(auto& p: list){
                    if(p.pid == pid){
                        p.response_ratio = (float)(curr_time - p.arrival_time + p.burst_time)/p.burst_time;
                        list_ready_queue.push_back(p);
                        break;
                    }
                }
                ready_queue.pop();
