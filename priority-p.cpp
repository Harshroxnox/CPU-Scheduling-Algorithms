...
using namespace std;
// priority preemptive higher number means higher priority
...
public:
    ...
    process(int id, int at, int bt, int _priority){
        pid = id;
        arrival_time = at;
        burst_time = bt;
        remaining_time = bt;
...
// Reverse comparison function for min attr in order to convert the max priority queue into min priority queue
...
void print_table(vector<process>& list){
    ...
    cout << line << endl;
    cout << "|\tPid\t|\tAT\t|\tBT\t|\tPr.\t|\tCT\t|\tTAT\t|\tWT\t|\tRT\t|" << endl;
...
int main(){
    ...
    for(int i=0; i<n; i++){
        ...
        cin >> pid >> at >> bt >> pr;
        list2.push_back(process(pid, at, bt, pr));
    }
